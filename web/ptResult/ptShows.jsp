...
<%
	...
	if(request.getParameter("pShowSeq")!=null)
        ...
        strEnvName=request.getParameter("PEnvName");	
    ...
    try {
        ...
        if(dbAction.equalsIgnoreCase("get")){
            cst=conn.prepareCall("{call OnlineGet_ShowInformationMobile(?,?,?,?,?)}");
            cst.setString(1, "");//ShowName,place,Date,@ShowSeq,@WorkOrderSeq
			cst.setString(2, "");
			cst.setString(3, "");
			cst.setString(4, "");
                        cst.setString(5, strShowIdent);
                       
        }

        rs=cst.executeQuery();    
		int i=0;
                if(strEnvName !=null && strEnvName!="" ){
                    System.out.println("Inside Tecch");
                    while(rs.next()){
                        
                   JSONObject obj = new JSONObject();
            if(strEnvName.equals(rs.getString("EnvironmentSeq"))){
                System.out.println("adada"+rs.getString("Show_Name"));
...
//            obj.put("showDay", rs.getString("ShowDay"));
           // obj.put("showMonth", rs.getString("ShowMonth"));
            obj.put("className", rs.getString("Show_Name"));
              obj.put("eName", rs.getString("Environment"));
              obj.put("eSeq", rs.getString("EnvironmentSeq"));
			  obj.put("highPts", rs.getString("HighPoints"));
            obj.put("stake","10.00");
