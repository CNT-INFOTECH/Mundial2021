...
rs = cst.executeQuery();
        ...
        while (rs.next()) {
			...
			if(Integer.parseInt(strslctid)==1)
		...
		if(rs.getString("PointsP")!=null)
            obj.put("GC", rs.getString("PointsP"));
			else
            obj.put("GC", "");
		
		if(rs.getString("PointsS")!=null)
		...
		if(rs.getString("Seq")!=null)
         ...
         //   System.out.println("after" +strslctid);
            ...
            cst = conn.prepareCall("{call GET_POINTS_MODALITY_WS_DETAILS(?,?,?,?,?)}");
             cst.setString(1,strShowSeq);
            cst.setString(2,strAire);
			cst.setString(3,strCategory);
			cst.setInt(4,Integer.parseInt(strslctid));
