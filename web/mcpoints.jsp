...
function Navigate(Show,varCountry,varCntryDesc)
{
		 ...
		 document.forms[0].method="post";
		  document.forms[0].action="mcpoints_details.jsp?cntry="+varCountry;
  ...
  {
	      document.forms[0].method="post";
		  document.forms[0].action="mcpoints.jsp"
		  document.forms[0].submit();
  }
...
<thead>
<th  data-class="expand" width="10%"></th><th  data-class="expand" width="10%">COUNTRY</th><th  data-class="expand" width="10%">POINTS</th>
  </thead>
  
  <%
		    localConnection=dbcmt.getConnection();
            cst = localConnection.prepareCall("{call GET_POINTS_Mundial_ByCountry_Equi(?)}");
            cst.setString(1,"24");
        ...
        {%>
			...
			<td style='text-align:center;'><%if (rs1.getString("Country")!=null && rs1.getString("Country").length()>0 &&  !rs1.getString("Country").equalsIgnoreCase("NONE")) {%><img src="img/<%=rs1.getString("Country")%>.jpeg"  width='15%'><%}%></td>
