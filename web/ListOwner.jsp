...
<script>
  ...
  {
 // window.history.go(-2);
 window.location.href = "http://pasotracker.com/Mundial2017/mClasses.html?sSq=61";
  }
  ...
  function RefreshResults(objValue)
  {
		  ...
		  document.forms[0].action="ListOwner.jsp"
		  document.forms[0].submit();
  }

  function PassKeyDetails(varShow,varSeq,varName)
	{


			document.forms[0].ShowSeq.value=varShow
			document.forms[0].Seq.value=varSeq
			document.forms[0].Name.value=varName
			



			document.forms[0].method="post";
			document.forms[0].action="ListDetails.jsp";
			document.forms[0].submit();
	}
...
<table align="center" class="tPasoHead" >
	   <thead>
       <th><font size="3" >View Points Owner/ Vea Puntos por Expositor</font></th>
       </thead> 
...
<table class="tPaso">
  ...
  <td <% if (strCategory.equals("Pasofino")) {%> bgcolor="#dce9f9" <%}%>>
           <input type="radio" value='Pasofino' name="chkCategory" onClick="RefreshResults(this.value)" <% if (strCategory.equals("Pasofino")) {%> checked <%}%>>Classic Paso fino</td>
...
<table class="tPaso">
      <thead>
<th data-class="expand" >OWNER NAME</th>
<!--<th  data-class="expand" >ID#</th>-->
<th data-class="expand" >POINTS</th>
<th data-class="expand" >GC</th>
<th data-class="expand" >GCR</th>
        ...
        try
        {
            ...
            CallableStatement cst = localConnection.prepareCall("{call GET_POINTS_MODALITY_WS(?,?,?,?)}");
        ...
        ResultSet rs1 = cst.executeQuery();
         ...
         while (rs1.next())
        {%>
             <tr style="cursor: pointer;" onClick="PassKeyDetails( <%=strShowSeq%>,'<%=rs1.getString("Seq") %>','<%=rs1.getString("Name")%>' ) ">
			...
			<td style="text-transform: uppercase"><%=rs1.getString("Points")%></td>
			<td style="text-transform: uppercase"><%=rs1.getString("PointsP")%></td>
			<td style="text-transform: uppercase"><%=rs1.getString("PointsS")%></td>
...
catch(Exception Ex)
{
  ...
  <input type="hidden" name="chkCategory" value='<%=strCategory%>' >
  <input type="hidden" name="expType" value='4' >
